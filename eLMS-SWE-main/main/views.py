...
from django.shortcuts import render, redirect
from django.contrib import messages
from .forms import CourseCreationForm, StudentRegistrationForm
...
def create_course(request):
    if request.method == 'POST':
        ...
        # Проверяем, что отдел выбран
        if not department_id:
            return render(request, 'create_course.html', {'error': 'Пожалуйста, выберите отдел.'})
        ...
        # Проверка уникальности facultyKey
        if Course.objects.filter(facultyKey=faculty_key).exists():
        ...
        # Перенаправляем на страницу курсов после сохранения
